...
impl SubsystemManager {
    ...
    pub async fn start_subsystem(&self, id: SubsystemId) -> Result<()> {
        ...
        #[cfg(feature = "tokio")]
        {
            ...
            let task = tokio::spawn(async move {
                let result = subsystem.run(shutdown_handle).await;
    ...
    pub async fn stop_all(&self) -> Result<()> {
        ...
        let stop_tasks: Vec<_> = subsystem_ids.into_iter()
    ...
    pub async fn restart_subsystem(&self, id: SubsystemId) -> Result<()> {
        ...
        if !delay.is_zero() {
            info!(subsystem_id = id, delay_ms = delay.as_millis(), "Waiting before restart");
    ...
    pub fn get_subsystem_metadata(&self, id: SubsystemId) -> Option<SubsystemMetadata> {
        let subsystems = self.subsystems.lock().unwrap();
        subsystems.get(&id).map(|entry| entry.metadata.lock().unwrap().clone())
    }
        ...
        for data in subsystem_data.iter() {
            ...
            let is_healthy = match state {
                SubsystemState::Running => {
                    ...
                    subsystem.health_check().map_or(true, |health_check| health_check())
                },
    ...
    fn should_restart(entry: &SubsystemEntry) -> bool {
        ...
        let (restart_policy, state, restart_count) = {
            ...
            (metadata.restart_policy, metadata.state, metadata.restart_count)
        };
    ...
    fn calculate_restart_delay(entry: &SubsystemEntry) -> Duration {
        ...
        match restart_policy {
            RestartPolicy::ExponentialBackoff { initial_delay, max_delay, .. } => {
...
mod tests {
    ...
    impl Subsystem for TestSubsystem {
        fn run(&self, mut shutdown: ShutdownHandle) -> Pin<Box<dyn Future<Output = Result<()>> + Send>> {
    ...
    async fn test_subsystem_start_stop() {
        ...
        let test_result = tokio::time::timeout(Duration::from_secs(5), async {
            ...
            let stop_result = tokio::time::timeout(Duration::from_millis(1000), manager.stop_subsystem(id)).await;
