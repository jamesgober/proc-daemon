...
async fn test_subsystem_execution_in_async_std() {
        ...
        async move {
            ...
            *executed.lock().unwrap() = true;
            ...
            async_std::future::timeout(
                ...
                shutdown.cancelled()
            ).await.ok(); // Ignore timeout result
    ...
    assert!(start_result.is_ok(), "Failed to start subsystem: {:?}", start_result);
    ...
    while !*executed.lock().unwrap() && attempts < 20 {
        ...
        println!("Attempt {}: Subsystem state: {:?}", attempts, metadata.state);
    }
    
    // Verify that the subsystem executed
    assert!(*executed.lock().unwrap(), "Subsystem did not execute within the expected time");
    ...
    assert!(coordinator.is_shutdown(), "Coordinator should be in shutdown state");
}
